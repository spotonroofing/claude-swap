...
Willem's fork (`0.27.0b3`) requires Fable for automatic selection by default, including bare `cswap switch` and both usage-aware strategies. The latest persisted usage reading must contain a scoped window named `Fable` (case-insensitive); no window means no Fable, and no reading means unknown. Both are skipped. These accounts keep being polled. When a plan gains or loses Fable, the next successful usage refresh replaces the stored reading and the very next rotation decision includes or excludes the account accordingly, subject to the usual quota and enable/disable rules. No restart, re-login, or manual command is needed. This does not change the separate `--model Fable` quota threshold setting.
...
Accounts without Fable show no Fable usage line and no plan label in list/status output; `cswap status --json` includes an `accounts` array with `has_fable: true`, `false`, or `null` per account (`cswap list --json` also includes it). Explicit `cswap switch <num|email>` and `cswap run <num|email>` still work without a no-Fable notice. `cswap disable` and `cswap enable` remain separate manual controls.
